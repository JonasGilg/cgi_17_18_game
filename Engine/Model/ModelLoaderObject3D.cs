...
namespace Engine.Model {
	...
	public static class ModelLoaderObject3D {
		...
		public static Model3D load(string filePath, float scale = 1.0f, bool doAverageTangets = false, bool createVAO=true, bool withNoise=false) {
			...
			if (withNoise) {
				...
				var freq = (float)Random.Next(1, 5) / 100;
				...
				if (parts.Length > 0) {
					switch (parts[0]) {
						case "v":
							...
							v.Add(Vector3.Multiply(vec3,noise));
							break;
						case "vt":
